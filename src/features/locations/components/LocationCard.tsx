    ...
    <Card className="group overflow-hidden flex flex-col h-full border-none shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-card">
      ...
      <CardFooter className="gap-3 pt-2">
        <Link href={`/locations/${location.id}`} className="flex-1">
           <Button variant="outline" className="w-full border-brand-maroon/20 hover:bg-brand-maroon/5 hover:text-brand-maroon group/btn">
             Details
           </Button>
        </Link>
        <Link href={`/booking?location=${location.id}`} className="flex-1">
           <Button className="w-full bg-brand-saffron hover:bg-brand-saffron/90 text-white shadow-sm hover:shadow-md transition-all">

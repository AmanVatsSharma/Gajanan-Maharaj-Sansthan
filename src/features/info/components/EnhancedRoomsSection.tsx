    ...
    >
      <div className="modal-content w-full sm:w-11/12 md:w-3/4 lg:w-2/3 rounded-2xl">
        <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex justify-between items-center z-10">
          <h2 className="text-2xl font-bold text-brand-maroon">{room.title}</h2>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {room.images.map((image, index) => (
                <Image
                  key={index}
                  src={image}
                  alt={`${room.title} view ${index + 1} - Interior of accommodation at Shri Gajanan Maharaj Sansthan`}
                  width={250}
                  height={192}
            ...
            <ul className="space-y-2 text-gray-700">
              {room.features.map((feature, index) => (
                <li key={index} className="flex items-center gap-2">
                  <svg className="w-5 h-5 text-brand-saffron" fill="currentColor" viewBox="0 0 20 20">
          ...
          {/* Price and CTA */}
          <div className="bg-linear-to-r from-brand-gold/10 to-brand-saffron/10 rounded-lg p-6 space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-gray-700 font-semibold">Price per night:</span>
              <span className="text-3xl font-bold text-brand-saffron">{room.currency}{room.price}</span>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              <a
                ...
                className="flex-1 px-6 py-3 bg-[#25D366] text-white rounded-lg font-semibold hover:bg-[#128C7E] transition-colors flex items-center justify-center gap-2"
              ...
              <a
                ...
                className="flex-1 px-6 py-3 bg-brand-saffron text-white rounded-lg font-semibold hover:bg-brand-maroon transition-colors flex items-center justify-center gap-2"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {ROOMS.map((room) => (
            ...
            >
              <div className="relative overflow-hidden h-48 bg-linear-to-br from-brand-gold/20 to-brand-saffron/10">
                <Image
                  src={room.image}
                ...
                <div className="absolute bottom-3 left-3 bg-linear-to-r from-green-600 to-emerald-600 text-white px-2.5 py-1.5 rounded-lg text-xs font-semibold shadow-lg flex items-center gap-1.5">
              ...
              <div className="p-4">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-2xl font-bold text-brand-saffron">{room.currency}{room.price}</span>
                  <button 
                    className="px-4 py-2 bg-brand-saffron text-white rounded-lg text-sm font-semibold hover:bg-brand-maroon transition-colors"
                    onClick={(e) => {
                      e.stopPropagation();
                      openModal(room);
                    }}
